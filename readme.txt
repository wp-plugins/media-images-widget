=== Plugin Name ===
Contributors: Florian Tobias
Tags: Widget, Image, Media Library
Requires at least: 3.0
Tested up to: 3.2.1
Stable Tag: 0.9.3

Requieres PHP 5!
You can select images from media Library in the Widgets backend that will be shown on your Page.

== Description ==
You can select images from media Library in the Widgets backend that will be shown on your Page. You can enter a size for the images.

The thumb-images will be linked to the full-sized-image.
Requieres PHP 5!

== Installation ==

1. Upload `ft-media-images-widget.php` or the plugin's directory to the `/wp-content/plugins/` directory
2. Activate the plugin through the 'Plugins' menu in WordPress
3. Enjoy!



== Screenshots ==

1. Widget's Backend
2. Widget on Frontpage


== Changelog ==
= 0.9.3 = 
* If there no images selected, the Widget will not produce errors on frontpage 
= 0.9.2 =
* Changed Backend-Thumbnail-Formatting
= 0.9.1 = 
* There was a Bug with the "posts_per_page"-Option. The Plugin will now shown at least 1000 images
= 0.9 =
* Added Image-Size